            ...
            <div className='container md:py-16 lg:my-0.5'>
                ...
                <div className='mt-6 sm:mt-10 lg:mt-[54px] grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 items-start'>
                    {TIMELINE_PHASE_DATA_LIST.map((item, index) => (
                        <div className='relative pl-10 sm:pl-0 h-full pb-8 sm:pb-12 xl:pb-0'>
                            <p key={index} className='font-luckiest text-2xl md:text-custom-4xl text-white !leading-3xl tracking-2xl mt-1.5 sm:mt-0'>Q{item.quarter}</p>
                            ...
                            <div className={`mt-4 sm:mt-8 xl:mt-[54px] sm:pr-6 ${index === 0 ? "xl:pr-8" : index === 1 ? "xl:pl-2 xl:pr-6" : "xl:px-4"}`}>
                                <p className='font-luckiest text-transparent stroke-2 stroke-white text-stroke uppercase !leading-2xl text-4xl sm:text-5xl md:text-custom-5xl font-normal tracking-3xl'>Phase {index + 1}</p>
                                <p className='font-comic font-bold text-white text-lg md:text-xl lg:text-custom-sm !leading-3xl mt-1.5 sm:mt-2 lg:mt-3 xl:mt-6'>{item.title}</p>
