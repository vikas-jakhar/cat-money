    ...
    return (
        <section className='lg:py-16 md:py-14 sm:py-12 py-10'>
            <div className="container pt-3 lg:mb-20 md:mb-10 sm:mb-5 pb-0.5">
                ...
                <div className="lg:mt-20 md:mt-16 sm:mt-14 mt-10 grid grid-cols-1 sm:grid-cols-2 justify-between gap-y-10 md:gap-y-12 lg:gap-y-16 gap-x-5 md:gap-x-10">
                    {THE_MAGIC_DATA_LIST.map((items, index) => (
                        <div key={index} className={`max-w-[534px] w-full flex gap-6 ${index === 1 || index === 3 || index === 5 ? "sm:ml-auto" : ""}`}>
                            <img src={items.image} alt="cat-icon" className='lg:w-24 lg:h-24 w-16 h-16' />
                            <div className="flex flex-col">
                                <p className='text-white font-comic font-bold text-xl lg:text-2xl !leading-3xl'>{items.title}:</p>
