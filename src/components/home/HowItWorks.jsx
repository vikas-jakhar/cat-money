                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 justify-between gap-x-10 pt-1 w-full">
                    {HOW_IT_WORKS_DATA_LIST.map((item, index) => (
                        <div key={index} className={`max-w-[522px] w-full relative sm:mt-12 mt-10 mx-auto ${index === 0 || index === 2 ? "sm:mx-0" : "mr-0"}`}>
                            <img fetchPriority='high' decoding='async' src={item.image} alt="cat" className='sm:max-w-[200px] pointer-events-none max-w-[150px] w-full mx-auto lg:mx-0' />
                            ...
                            {index === 2 && <img fetchPriority='high' decoding='async' src={redLineDot} alt='redDot' className='-top-11 left-0 lg:-left-20 pointer-events-none absolute' />}
                        </div>
                    ))}
                </div>
            </div>
