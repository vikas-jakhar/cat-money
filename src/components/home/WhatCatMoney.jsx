                ...
                <div className="flex justify-between items-center gap-5 lg:flex-row flex-col">
                    <div className="max-w-[630px] w-full relative z-10">
                        ...
                        {WHAT_CAT_MONEY_DATA_LIST.map((items, index) => (
                            <Paragraph key={index} className={`${index === 0 || index === 7 ? "md:mt-5 sm:mt-4 mt-2" : index === 2 ? "md:mt-7 sm:mt-5 mt-3 !font-bold" : index === 6 ? "!font-bold md:mt-5 sm:mt-4 mt-2 flex items-center gap-4 !text-start" : "md:mt-7 sm:mt-5 mt-3"} text-center lg:text-left`}>{index === 3 && <span className='font-bold'>Every week</span>}{index === 6 && <img fetchPriority='high' decoding='async' src={jackpot} alt='jackpot' className='w-10 pointer-events-none h-10' />}{items}.</Paragraph>
                        ))}
                    </div>
                    <div className="max-w-[522px] w-full relative mt-7 lg:mt-0">
                        <img fetchPriority='high' decoding='async' src={gameCat} alt="gameCat" className='w-full relative z-10 pointer-events-none' />
                        <img fetchPriority='high' decoding='async' src={gameFootprint} alt="gameFootprint" className='-top-[47px] pointer-events-none -left-[76px] absolute' />
                    </div>
                </div>
            </div>
            <img fetchPriority='high' decoding='async' src={catFootprint} alt="catFootprint" className='-bottom-14 right-5 pointer-events-none absolute z-10 hidden sm:flex' />
        </section >
    )
}
