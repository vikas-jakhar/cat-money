                ...
                <div className="mt-5 sm:mt-8 lg:mt-[53px] flex flex-col gap-3 sm:gap-4">
                    {FAQS_DATA_LIST.map((item, index) => (
                        <div key={index} className="bg-light-black !duration-500 transition-all">
                            <button onClick={() => handleToggle(index)} className="flex items-center p-5 sm:p-6 lg:px-10 lg:py-8 justify-between gap-5 w-full">
                            ...
                            <div className={`grid overflow-hidden !duration-500 transition-all ${activeIndex === index ? "grid-rows-[1fr] opacity-100 h-fit max-h-[500px]" : "max-h-0 grid-rows-[0fr] opacity-0"}`}>
                                <p className={`font-barlow text-base sm:text-lg text-white font-light md:text-xl flex p-5 sm:p-6 lg:px-10 lg:py-8 !pt-0 flex-col gap-3 !leading-6xl ${index === 4 ? "" : "lg:gap-7"}`}>{item.answer}
